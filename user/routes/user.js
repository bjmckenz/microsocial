...
router.get("/user/:id", (req, res) => {
  ...
  const stmt = db.prepare("SELECT id, name, phone_number, status, created FROM users where id = ?");
  users = stmt.all([id]);
  ...
  delete user.status
  user.status = status == 0 ? "disabled" :"active"
  res.json(user);
});
 ...
 * /user/{id}/phone:
 *   get:
 *     summary: Retrieve the phone number from a user
 *     description: Retrieve one phone number by id.
 *     operationId: GetUserById
 *     tags: [Users API]
 *     parameters:
 *       - in: path
 ...
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/RetrievedUserPhoneNumber'
 *       404:
 *         description: No such User
 *         examples: [ "Not Found", "No such user" ]
 */
router.get("/user/:id/phone", (req, res) => {
  const id = parseInt(req.params.id);
  ...
  const stmt = db.prepare("SELECT phone_number FROM users where id = ?");
  user = stmt.all([id]);
  ...
  user.uri = USERS_SERVICE(`/user/${id}/phone`);
  res.json(user);
});
 ...
 * /user/{id}/status:
 *   get:
 *     summary: Retrieve the status from a user
 *     description: Retrieve status of user, by id.
 *     operationId: GetUserById
 *     tags: [Users API]
 *     parameters:
 *       - in: path
 ...
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/RetrievedUserStatus'
 *       404:
 *         description: No such User
 *         examples: [ "Not Found", "No such user" ]
 */
router.get("/user/:id/status", (req, res) => {
  ...
  const stmt = db.prepare("SELECT status FROM users where id = ?");
  users = stmt.all([id]);
  ...
  user.uri = USERS_SERVICE(`/user/${id}/status`);
...
router.post("/user/:id", (req, res) => {
  ...
  // Do this to avoid the status is null
  sql = "UPDATE users SET name=?, password=?, phone_number=?, status=? WHERE id=?"
  if(updatedUser.status == null){
    sql = "UPDATE users SET name=?, password=?, phone_number=? WHERE id=?"
  }
  ...
  try {
    if(updatedUser.status == null){
      info = stmt.run([updatedUser.name, updatedUser.password, updatedUser.phone_number, id]);
    } else{
      info = stmt.run([updatedUser.name, updatedUser.password, updatedUser.phone_number, updatedUser.status, id]);
    }
    if (info.changes < 1) {
      log_event({
        severity: 'Low',
...
router.patch("/user/:id", (req, res) => {
  ...
  try {
    ...
    if ("phone_number" in updatedUser){
      updateClauses.push("phone_number = ?");
